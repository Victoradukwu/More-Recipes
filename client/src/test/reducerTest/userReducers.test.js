...
describe('userAuthentication', () => {
  ...
  it('returns the expected state for a SET_USER action', () => {
    const newState = reducer(initialState, userActions.setUser({ id: 1, name: 'Archer' }));
    expect(newState).toEqual({
      ...initialState,
  ...
  it('returns the expected state for a AUTHENTICATE_USER_FAILURE action', () => {
    const newState = reducer(initialState, userActions.userAuthenticationFailure('someError'));
    expect(newState).toEqual({
      ...initialState,
      authError: 'someError'
    });
